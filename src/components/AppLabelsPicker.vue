...
<template>
  <div>
    ...
    <button v-for="(label, index) in labels" :key="label.id || label.label"
      :class="`bg-${label.color}-500 p-2 rounded text-white my-1 flex justify-between`">
      <div class="flex items-center">
        <span @click.prevent="handleToggle(label)" class="w-4 h-4 mr-2" :class="
          selected?.map((s) => s.id).includes(label.id)
            ? `k-icon k-i-check`
            : ''
        "></span>
        <input class="w-3/4 bg-transparent ouline-none" type="text" :value="label.label" @change="
          handleUpdate(($event.target as HTMLInputElement).value, index),
          ($event.target as HTMLInputElement).blur()
        " />
      </div>
      <button class="k-icon k-i-trash" @click.prevent="handleDelete(label)"></button>
    </button>
    <div>
      ...
      <div v-if="showCreate">
        ...
        <KButton @click="handleCreate" class="block mt-3" theme-color="primary">Create</KButton>
      </div>
    </div>
  </div>
</template>